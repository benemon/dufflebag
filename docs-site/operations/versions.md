...
The Security card reads the package inventory once per sign-in and keeps the
eight most recently read builds for the session: opening a build and
returning, or revisiting the version, reuses the read. **Refresh** re-reads it,
which is how a rescan's findings appear. Large images can take a minute or more; the card reports the number of
