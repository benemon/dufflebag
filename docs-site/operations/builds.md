...
inventory on the build screen's **Packages** tab. The inventory is read once
per sign-in and shared with the version screen for the rest of the session;
**Refresh** re-reads it. Large images can take a minute or more, so the card
reports how many packages it has read while the request is in progress.
